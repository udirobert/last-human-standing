...
| 2026-07-10 | (pending deploy) | **Focus + mythmaking.** Onboarding rules slimmed to core loop; `ROUND_UNLOCKS` + `RuleReveal` teach one twist per day (infiltrator / pressure / wildcard / finale). Mission mantra on live home; arsenal gated on play progress and moved below feed with prize pots; `SpectatorChip` = cohort-2 priority only. **Shareable moment cards:** canvas PNG for survive/jury/win + `/api/share/winner` OG. Docs aligned. |
