...
export default function LandingHero({ targetIso, reservedCount = 0, cohortSize = 50, onReserve, onDetails, onSpeedRun }) {
  ...
  const { unlockAndStart } = useDelight();
  const startSoundtrack = () => unlockAndStart?.({ phase: "prelaunch", screen: "home" });
    ...
    >
      ...
      >
        ...
        >
          <button
            type="button"
            onClick={() => { startSoundtrack(); onReserve?.(); }}
            {...CUE_PRESS}
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-center gap-2.5 w-full">
            <button
              type="button"
              onClick={() => { startSoundtrack(); onDetails?.(); }}
              {...CUE_HOVER}
            ...
            {onSpeedRun && (
              <button
                type="button"
                onClick={() => { startSoundtrack(); onSpeedRun?.(); }}
                {...CUE_PRESS}
