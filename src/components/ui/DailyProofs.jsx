...
/**
 * DailyProofs — the place-type wheel for the landing narrative
 * (docs/ART_DIRECTION.md). Each day a riddle drops; this shows the kinds of
 * places a riddle might accept, so a visitor instantly gets the game, and
  ...
  return (
    <section className="w-full max-w-[860px] mx-auto px-5">
      <div className="text-center mb-6">
        <p className="font-mono text-amber/90 uppercase text-[11px] tracking-[0.2em]">Every day</p>
        <h2
          className="font-display text-bone tracking-wide mt-1"
          style={{ fontSize: "clamp(30px,5vw,52px)", lineHeight: 0.95 }}
        >
          A new riddle to answer
        </h2>
        <p className="font-body text-dim text-sm sm:text-base mt-2 max-w-md mx-auto">
          Each day a riddle drops — these are the kinds of places it might lead you. Which one it asks for stays secret until it drops. Tap a card to see what it could accept.
        </p>
      </div>
        ...
        {DAILY_THEMES.map((t) => (
          <li key={t.id}>
            <button
              ...
              className="w-full h-full flex flex-col items-center justify-end gap-1.5 rounded-2xl bg-smoke/50 border border-ember/40 py-4 px-2 transition-[transform,border-color] duration-150 active:scale-[0.96] hover:border-ember relative"
